...
const TOOL_DEFINITIONS: Record<string, ToolDefinition> = {
  ...
  'day-activity': {
    ...
    task: [
      ...
      'For "recommendations": leave `reply` null. Suggest EXACTLY 3 ideas — never more. The goal is the 3 BEST fits for what they actually asked, not a broad spread for variety\'s sake: if two similar ideas both genuinely fit better than a third, weaker, "different for the sake of different" one, give the two strong similar ones priority over padding it out with a worse but different-flavoured third. If they ask for more ("what else", "give me more ideas"), suggest 3 additional NEW ones, never repeating an idea already given earlier in this conversation.',
      ...
      'title is the activity idea in a short natural phrase — this is shown only until the app resolves it to a real nearby venue (then the real venue\'s own name takes over as the title), so it doesn\'t need to be polished or clever, just clear. category is a short type label (e.g. "Outdoors", "Food & Drink", "Culture", "Family", "Nightlife", "Relaxation"). duration is a rough time estimate (e.g. "1-2 hours", "Half day") — leave it null if it genuinely varies too much to say. description is one or two sentences on what it actually involves. suitableFor is a short phrase on who it best suits (e.g. "Couples looking for something active", "Families with young kids", "A relaxed solo afternoon") — grounded in what THEY actually asked for, not a generic audience blurb. whyRecommended is one short sentence on why it fits what they asked for specifically — not a generic blurb, and not a repeat of suitableFor.',
      'searchQuery is separate from title — a short, plain real-world search phrase for this activity TYPE (2-4 words, a genuine place/category search a mapping app would understand, e.g. "escape room", "coastal walk", "board game cafe", "bowling alley" — not creative or specific, and not the same wording as title if title is more flavourful, e.g. title "Solve the Mystery" but searchQuery "escape room"). The app uses it to find a REAL nearby venue matching your idea; never leave it null for a "recommendations" reply.',
