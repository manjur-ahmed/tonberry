...
interface TopicItemData {
  title: string
  sections: { heading: string; body: string; articles?: { url: string }[] }[]
}
...
function NewsItemView({ title, data, pinned }: { title: string; data: unknown; pinned?: boolean }) {
  ...
  const articleCount = data.sections.reduce((total, section) => total + (section.articles?.length ?? 0), 0)
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      ...
      <p className="mt-0.5 text-xs uppercase tracking-wide text-slate-400">
        ...
        {articleCount > 0 && ` · ${articleCount === 1 ? '1 article' : `${articleCount} articles`}`}
      </p>
