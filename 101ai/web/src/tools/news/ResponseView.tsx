...
// subheadings, not a pile of items each overwriting the last. Previously
// News saved one item per search instead (real GDELT results, no AI
// commentary) — that design left with the GDELT-backed live-search
// version; this is a plain generateReply tool now, same shape as every
// other topic-explainer tool (see tool-config.ts's news entry), with one
// addition: each new section fetches a handful of real articles (see
// lib/news.ts) as "further reading" — the AI writes the summary/key
// facts, real articles are what backs "read more" if the user wants it.

// A single turn's reply — what tool-config.ts's NEWS_SCHEMA produces for
// one message. searchKeywords is a short real search phrase for the
// further-reading fetch below — never shown to the user, and distinct
// from topicTitle/sectionHeading (both meant for display, not search).
interface TopicTurn {
  topicTitle: string
...
// `sections`, never has `kind`. Each section's `articles` (if any) is
// exactly what was fetched and saved when that section was first written —
// never re-fetched on reopen, so "further reading" stays what it was at
// save time.
interface TopicDocument {
  title: string
  sections: { heading: string; body: string; articles?: NewsArticle[] }[]
}
...
function isTopicTurn(value: unknown): value is TopicTurn {
  ...
  return (
    ...
    // searchKeywords deliberately not required here — a message saved
    // before this field existed just falls back to topicTitle below.
  )
}
...
function ArticleList({ articles }: { articles: NewsArticle[] }) {
  return (
    <div className="mt-3 space-y-2">
      <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Further reading</p>
      {articles.map((article) => (
        ...
        >
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-slate-500">{article.sourceName}</p>
          </div>
          <ExternalLink className="mt-0.5 h-4 w-4 flex-shrink-0 text-slate-400" strokeWidth={1.75} />
        </a>
      ))}
    </div>
  )
...
function NewsResponse({ content, toolSlug, chatId, messageId, readOnly, onSaveStatusChange }: ResponseViewProps) {
  ...
  // Only a brand-new section searches for real articles — a 'continue'
  // turn just adds detail to an already-settled heading, whose original
  // articles (already saved on that section) are still the relevant ones.
  // Queries on searchKeywords (a short real search phrase the model wrote
  // specifically for this — see tool-config.ts), falling back to
  // topicTitle for a message saved before that field existed. Confirmed
  // empirically against the real API that a longer, sentence-like phrase
  // either matched nothing or pulled in unrelated noise.
  const { user } = useAuth()
  const shouldFetchArticles = turn?.sectionAction === 'new' && !readOnly
  const searchQuery = turn ? (turn.searchKeywords || turn.topicTitle) : ''
  const articlesQuery = useQuery({
    queryKey: ['news-articles', searchQuery, user?.country ?? ''],
    queryFn: () => fetchNewsArticles(searchQuery, user?.country),
    enabled: shouldFetchArticles,
    staleTime: Infinity,
  })
  const articlesReady = !shouldFetchArticles || !articlesQuery.isPending
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      const request = upsertItemSection(toolSlug, chatId, {
        ...
        articles: turn.sectionAction === 'new' ? (articlesQuery.data ?? []) : undefined,
      })
  ...
  // prevents that, not just StrictMode noise. Waits on articlesReady so a
  // 'new' section saves with its real fetched articles rather than racing
  // the search and saving an empty list.
  //
        ...
        <div className="mt-4 space-y-6">
          {document.sections.map((section, index) => (
            <div key={index}>
              ...
              {section.articles && section.articles.length > 0 && <ArticleList articles={section.articles} />}
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div>
      ...
      {shouldFetchArticles && articlesQuery.data && articlesQuery.data.length > 0 && (
