...
// ItemsService.upsertSection), same reasoning as business-research —
// several angles/documents covered in one chat read back as one document
// with subheadings, not a pile of items each overwriting the last.

// A single turn's reply — what tool-config.ts's DOCUMENT_EXPLAINER_SCHEMA
...
// `kind: 'chat'` replies (greetings, small talk, or "no document attached
// yet" — see tool-config.ts) carry the same envelope with the rest null and
// the reply text in `reply` instead; only an 'explanation' reply renders as
// a turn or gets auto-saved.
...
function DocumentExplainerResponse({ content, toolSlug, chatId, messageId, readOnly, onSaveStatusChange, onCopyTextChange }: ResponseViewProps) {
  ...
  // The full saved document (see ItemDetailModal) — every angle/document
...
export default DocumentExplainerResponse
