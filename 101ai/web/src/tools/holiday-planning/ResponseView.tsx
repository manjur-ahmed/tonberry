...
interface HolidayPlan {
  // Stable per-trip id assigned by the model (see tool-config.ts) — same
  // mechanism as diet's planKey. Reused across every reply that amends
  // the same trip, only changing when the user asks about a genuinely
  // different one. Combined with chatId for the actual dedup key, so two
  // different trips in one chat don't collapse into a single item.
  planKey: string
  planTitle: string
  // A short supplementary aside alongside the plan (see tool-config.ts) —
  // most often the answer to a direct question asked in the same message
  // as the plan request (e.g. "...and do I need a visa?"), which would
...
// `kind: 'chat'` replies (getting to know their plans, small talk, follow-up
...
function isHolidayPlan(value: unknown): value is HolidayPlan {
...
// (e.g. "Day 2 – Colosseum" for a day-by-day itinerary, just "Flights" for
// a plain budget line item), and the last column is the value after it.
...
// Sub-points, but only when they'd actually save repetition — a day-by-day
// itinerary reads much better as "Day 2" with Colosseum/Lunch/Trevi Fountain
// nested under it than as three separate "Day 2 – Colosseum", "Day 2 –
...
function groupRows(columns: string[], rows: string[][]): PlanGroup[] | null {
  ...
  // label doesn't collapse into a single group merging stops from every
  // day together.
  let lastLabel = ''
...
function HolidayPlanningResponse({ content, toolSlug, chatId, messageId, readOnly, onSaveStatusChange }: ResponseViewProps) {
  ...
  try {
    ...
    if (isHolidayPlan(parsed)) data = parsed
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      // ResponseView for why (a title can change as the trip gets amended,
...
export default HolidayPlanningResponse
