...
// Plain AI-guidance tool — no live pricing/provider API behind it (see
// tool-config.ts's honesty guardrail: real quotes/deals are never
// presented as confirmed current data). Same topic-explainer shape as
// Politics/General Health: saves one item per *chat*, appended to over
// time, so a rabbit-hole conversation covering several bills/questions
// reads back as one document with subheadings.
...
function BillsUtilitiesResponse({ content, toolSlug, chatId, messageId, readOnly, onSaveStatusChange, onCopyTextChange }: ResponseViewProps) {
...
export default BillsUtilitiesResponse
