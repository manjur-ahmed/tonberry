...
interface BudgetPlan {
  // Stable per-budget id assigned by the model (see tool-config.ts) — same
  // mechanism as diet's planKey. Reused across every reply that amends
  // the same budget, only changing when the user wants to start a
  // genuinely different one. Combined with chatId for the actual dedup
  // key, so two different budgets in one chat don't collapse into a
  // single item.
  planKey: string
  planTitle: string
  // A short supplementary aside alongside the plan (see tool-config.ts) —
  // most often a pointer to where the user can find an actual figure
  // (bank statements, direct debit list), or a flag that the categories
  // drafted so far don't yet add up to their income (the zero-based check
  // this tool is built around) — either way, something that would
  // otherwise have nowhere to go since `reply` is null on a plan turn.
  note: string | null
  columns: string[]
  rows: string[][]
}

// `kind: 'chat'` replies (getting to know their goals and income, asking
// about costs — see tool-config.ts) carry the same envelope with the rest
...
// (e.g. "Needs – Rent" for a Type/Category budget), and the last column is
// the value after it.
...
// Sub-points, but only when they'd actually save repetition — a Type/
// Category budget reads much better as "Needs" with Rent/Groceries/Utilities
// nested under it than as three separate "Needs – Rent", "Needs –
// Groceries" bullets. Needs 3+ columns (nothing left to nest under 1-2)
// *and* the first column repeating across rows — a table where every row's
// first cell is already unique gains nothing from grouping, so it stays
// flat.
function groupRows(columns: string[], rows: string[][]): PlanGroup[] | null {
  if (columns.length < 3) return null
  // A blank leading cell is the model's way of saying "same type as the
  ...
  // type label doesn't collapse into a single group merging categories
  // from every type together.
  let lastLabel = ''
...
function BudgetPlannerResponse({ content, toolSlug, chatId, messageId, readOnly, onSaveStatusChange, onCopyTextChange }: ResponseViewProps) {
  let data: BudgetPlan | null = null
  let chatReply: string | null = null
  try {
    const parsed = JSON.parse(content)
    if (isBudgetPlan(parsed)) data = parsed
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      // ResponseView for why (a title can change as the budget gets
      // amended, planKey is what stays stable across that).
  ...
  return (
    <div>
      ...
      {data.note && (
        <p className="mt-2 rounded-xl bg-violet-50 p-3 text-sm text-slate-700">{data.note}</p>
      )}
...
export default BudgetPlannerResponse
