...
// amend-in-place mechanism as cooking's dishKey/diet's planKey (see
// business-plan/ResponseView.tsx, this tool's closest sibling in shape).
// Every reply is the COMPLETE current plan, not a diff, so the saved item's
// `data` is always this same shape.
interface EventPlan {
  // Stable per-event id assigned by the model (see tool-config.ts) — same
  // mechanism as cooking's dishKey. Reused across every reply that amends
  // the same event, only changing when the user asks about a genuinely
  // different one. Combined with chatId for the actual dedup key, so two
  // different events in one chat don't collapse into a single item.
  planKey: string
  planTitle: string
...
function EventPlannerResponse({ content, toolSlug, chatId, messageId, readOnly, onSaveStatusChange }: ResponseViewProps) {
  ...
  try {
    ...
    if (isEventPlan(parsed)) data = parsed
...
export default EventPlannerResponse
